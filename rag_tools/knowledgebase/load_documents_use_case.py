...
def execute(data_dir: str, text_splitter: TextSplitter) -> List[Document]:
    ...
    for file_path in tqdm(files, desc="Processing Files", unit="file"):
        ...
        try:
            ...
            print(f"Loaded {len(new_documents)} documents from {file_path}")
        except Exception as e:
            print(f"Error loading {file_path}, skipping. {e}")
        if text_splitter:
            document_chunks = _split_documents(text_splitter, new_documents)
            print(f"Document {file_path} split into {len(document_chunks)} chunks.")
...
def _split_documents(text_splitter: TextSplitter, documents: List[Document]) -> List[Document]:
    document_chunks = []
    for document in documents:
        try:
            ...
            print(f"Error splitting document {document.metadata.get('source', 'unknown')}: {e}")
